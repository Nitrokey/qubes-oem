...
class Users(object):
    ...
    def setLuksPassword(self, lukspassword):
        """Set a new lukspassword for hard-coded dom0 device
        """
        f = open("/dev/shm/oldpass", "w")
        f.write('')
        f.close()
        f = open("/dev/shm/newpass", "w")
        f.write(lukspassword)
        f.close()

        try:
            iutil.execWithRedirect("cryptsetup", ["luksChangeKey", "--key-file", "/dev/shm/oldpass", "/dev/sda2", "/dev/shm/newpass"])
            iutil.execWithRedirect("shred", ["-u", "/dev/shm/oldpass", "/dev/shm/newpass"])
        except OSError as e:
            log.critical("Unable to change luks password: %s", e.strerror)
            raise
